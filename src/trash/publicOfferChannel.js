...
class PublicOfferChannel extends Session {
  constructor (orbitdbC, db, capability, offer) {
    super(db, capability, offer)
  ...
  // session type
  static get type () { return 'public-offer-channel' }
  ...
  static async open (orbitdbC, address, capability, options) {
    ...
    await this._verifyOffer(db)
    return new PublicOfferChannel(orbitdbC, db, capability)
  }

  static async offer (orbitdbC, name, options) {
  ...
  static async accept (orbitdbC, idKey, address, options) {
  ...
  static async _verifyOffer (offer) {
    if (!offer) throw new Error('offer is undefined')
    const { id, type, ttl, handshake } = offer
    if (!id || !type || !ttl || !handshake) {
      throw new Error('offer missing field')
    }
  }
  ...
  */
  ...
  async sendOffer (offer) {
    ...
    this.events.emit('sendOffer', iv.join(''))
  ...
  async _aesKey (requestId) {
    ...
    const ecdh =
      await crypto.ecdh.import(this._capability.curve, this._capability.jwk)
...
module.exports = PublicOfferChannel
